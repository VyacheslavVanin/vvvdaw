#pragma once
#include <cstdint>
#include <memory>
#include <vector>
#include <QJsonObject>
#include "MidiClip.h"

class MidiEvent {
public:
    MidiEvent() = default;

    bool isValid() const { return static_cast<bool>(activeClip()); }
    int64_t endSample() const;

    void addTake(std::shared_ptr<MidiClip> takeClip);
    void setActiveTake(int index);
    const std::shared_ptr<MidiClip>& activeClip() const;

    int64_t id() const { return m_id; }
    void setId(int64_t id) { m_id = id; }

    const std::shared_ptr<MidiClip>& clip() const { return m_clip; }
    void setClip(const std::shared_ptr<MidiClip>& clip) { m_clip = clip; }

    int64_t startSample() const { return m_startSample; }
    void setStartSample(int64_t s) { m_startSample = s; }

    int64_t offsetSample() const { return m_offsetSample; }
    void setOffsetSample(int64_t s) { m_offsetSample = s; }

    int64_t durationSample() const { return m_durationSample; }
    void setDurationSample(int64_t s) { m_durationSample = s; }

    const std::vector<std::shared_ptr<MidiClip>>& takes() const { return m_takes; }
    std::vector<std::shared_ptr<MidiClip>>& takes() { return m_takes; }

    int activeTakeIndex() const { return m_activeTakeIndex; }
    void setActiveTakeIndex(int idx) { m_activeTakeIndex = idx; }

    MidiEvent cloneDeep() const;

    QJsonObject toJson() const;
    static MidiEvent fromJson(const QJsonObject& obj);

private:
    int64_t m_id = 0;
    std::shared_ptr<MidiClip> m_clip;
    int64_t m_startSample = 0;
    int64_t m_offsetSample = 0;
    int64_t m_durationSample = 0;

    std::vector<std::shared_ptr<MidiClip>> m_takes;
    int m_activeTakeIndex = -1;
};
